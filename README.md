...
**Nowhere except Octopus and default-on anonymous PostHog event sharing.** This is a static HTML file with no backend and no server of any kind. Energy data requests go directly from your browser to `api.octopus.energy` using your own API key. The page loads no external scripts, styles, fonts, or images — open your browser's network tab to verify, which is the whole point of it being one inspectable file.

- **Analytics (default-on, can be unticked before running):** if you leave "Share anonymous results with Auth Energy" ticked on the connect screen, one event is sent to [PostHog](https://posthog.com/) (EU-hosted, `eu.i.posthog.com`) when a comparison completes. It contains only: your postcode's **outward area** (e.g. "SW1"), the **percentage difference** between tariffs, **total kWh** used, and the **period length in days**. If a comparison fails, a second event records only the error type, HTTP status code, whether it looks CORS-related, and which stage failed. No API key, account number, MPAN, meter serial, full postcode, raw consumption, tariff data, dates, or £ amounts are ever sent. The app sends these events directly without PostHog cookies, browser persistence, autocapture, page views, feature flags, or session recording. Untick the checkbox before running to send nothing at all.
- **Storage:** the theme and analytics sharing preference may be saved in your browser's `localStorage`. Your **API key** is saved only if you tick **"remember my key"** — also to `localStorage` (not a cookie, never transmitted), until you clear it. Your account number, MPAN, and meter serial are discovered from Octopus each run and are **never** stored.
