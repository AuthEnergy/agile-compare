...
// account/mpan/serial are now auto-discovered via fetchAndPickMeter in production;
// in tests we pre-populate state directly to mirror what that function does.
...
(async () => {
  ...
  // Pre-populate the fields that fetchAndPickMeter normally sets after
  // auto-discovering the account — in tests we set them directly.
...
(async () => {
  ...
  // Pre-populate state for the second scenario
  state.accountNumber = "A-TEST0001";
  state.mpan = "1234567890123";
  state.serial = "12A3456789";
  state.accountData = {
  ...
  const content2 = document.getElementById("results-content").innerHTML;
  assert.ok(!content2.includes("error-banner"), "a 14-month-old statement should not error");
  assert.ok(!content2.includes("partial"), "a 14-month-old statement should NOT be flagged as partial — this is the bug that was reported and fixed");
